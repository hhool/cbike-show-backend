...
const SYSTEM_KEYS = ["single", "compare", "newbie", "cross_border", "value", "debunk", "ranking"] as const;
...
export const ReviewCategories: CollectionConfig = {
  ...
  labels: {
    singular: { en: "Review Category", zh: "评测分类" },
    plural: { en: "Review Categories", zh: "评测分类" },
  },
  admin: {
    useAsTitle: "key",
    group: { en: "Editorial", zh: "内容编辑" },
    defaultColumns: ["key", "name", "isSystem", "isVisibleInTabs", "sortOrder", "updatedAt"],
    description: "用于评测中心后台归类与前台分类展示；建议 key 与评测 type 保持一致。",
  },
  access: {
    read: () => true,
    readVersions: () => true,
        ...
        if (isSystem && !SYSTEM_KEYS.includes(key as (typeof SYSTEM_KEYS)[number])) {
          throw new ValidationError({
            ...
            errors: [
              {
                path: "key",
                message: "系统评测分类仅允许使用预设 key。System review categories must use reserved keys.",
              },
            ],
            req,
          });
  ...
  fields: [
    {
      ...
      admin: { description: "例如 single/compare/newbie；建议与评测 type 对应。" },
    },
    {
      name: "name",
      type: "text",
