        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-start gap-4">
                <div className="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden shrink-0">
                  <Image src={company.logo} alt={company.nameEn} width={80} height={80} className="w-full h-full object-cover" unoptimized />
                </div>
                <div>
                  <h1 className="text-xl font-bold text-gray-900">{isEn ? company.nameEn : company.name}</h1>
                  <p className="text-sm text-gray-500 mt-0.5">{isEn ? company.name : company.nameEn}</p>
              ...
              <p className="text-sm text-gray-600 mt-4 leading-relaxed">{isEn ? company.introEn : company.intro}</p>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {company.gallery.map((img, i) => (
                    <div key={i} className="aspect-[4/3] rounded-lg overflow-hidden bg-gray-100">
                      <Image src={img} alt={`${company.nameEn} ${i + 1}`} width={300} height={225} className="w-full h-full object-cover" unoptimized />
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {companyProducts.map((p) => (
                    <Link key={p.id} href={`/${locale}/products/${p.slug}`} className="group rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition-shadow">
                      <div className="aspect-[4/3] overflow-hidden bg-gray-50">
                        <Image src={p.images[0]} alt={p.nameEn} width={200} height={150} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
                      </div>
                      <div className="p-3">
                        <div className="text-xs font-medium text-gray-800 line-clamp-2">{isEn ? p.nameEn : p.name}</div>
          ...
          <aside className="w-full lg:w-72 shrink-0 space-y-4">
            ...
            <div className="bg-blue-700 rounded-xl p-5 text-white">
              <h2 className="font-semibold mb-1">{d.inquiryCta}</h2>
              <p className="text-xs text-blue-200 mb-4">{isEn ? "Fill in your requirements and get a quick response." : "填写需求，我们将安排工厂快速回复"}</p>
              <Link href={`/${locale}/contact?company=${company.slug}`} className="block w-full bg-yellow-400 text-blue-900 text-center text-sm font-semibold py-2.5 rounded-lg hover:bg-yellow-300 transition-colors">
                {d.inquiryCta}
              </Link>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
