...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: product ? `${product.name} | 曲周童车产业带` : "产品详情" };
}
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="w-full md:w-80 shrink-0">
                  <div className="aspect-[4/3] rounded-xl overflow-hidden bg-gray-50">
                    <Image src={product.images[0]} alt={product.name} width={400} height={300} className="w-full h-full object-cover" unoptimized />
                  </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      {product.images.slice(1).map((img, i) => (
                        <div key={i} className="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 border-2 border-transparent hover:border-blue-400 cursor-pointer">
                          <Image src={img} alt={`${product.name} ${i + 2}`} width={64} height={64} className="w-full h-full object-cover" unoptimized />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex-1">
                  <div className="text-xs text-blue-600 font-medium mb-1">{product.categoryLabel}</div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">{product.intro}</p>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {relatedProducts.map((p) => (
                    <Link key={p.id} href={`/products/${p.slug}`} className="group rounded-xl overflow-hidden border border-gray-100 hover:shadow-md transition-shadow">
                      ...
                      <div className="p-2">
                        <div className="text-xs text-gray-800 line-clamp-2">{p.name}</div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden shrink-0">
                    <Image src={company.logo} alt={company.name} width={40} height={40} className="w-full h-full object-cover" unoptimized />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-gray-800">{company.name}</div>
                    <div className="text-xs text-gray-400">{company.region} · {company.categoryLabel}</div>
                  </div>
                </div>
