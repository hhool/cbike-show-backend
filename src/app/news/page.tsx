        ...
        <Link href={`/news/${featured.slug}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md border border-gray-100 transition-shadow mb-8">
          <div className="flex flex-col md:flex-row">
            <div className="md:w-1/2 aspect-[16/9] md:aspect-auto overflow-hidden">
              <Image src={featured.featuredImg} alt={featured.title.zh} width={800} height={450} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
            </div>
            <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
              <div className="flex gap-2 mb-3">
                <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">{featured.category}</span>
                <span className="text-xs bg-red-50 text-red-500 px-2 py-0.5 rounded-full">精选</span>
              </div>
              <h2 className="text-xl font-bold text-gray-900 mb-3 line-clamp-2">{featured.title.zh}</h2>
              <p className="text-gray-500 text-sm line-clamp-3 mb-4">{featured.excerpt.zh}</p>
              ...
              <div className="p-4">
                <div className="flex gap-2 mb-2 flex-wrap">
                  ...
                  {a.tags.slice(0, 2).map((t) => (
                    <span key={t.zh} className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{t.zh}</span>
                  ))}
                </div>
